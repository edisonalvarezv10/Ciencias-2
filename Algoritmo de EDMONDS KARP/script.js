...
// Variables globales para almacenar resultados de Edmonds-Karp
...
function calcularFlujoMaximo() {
    ...
    if (currentEdges.some(([, , capacity]) => capacity <= 0)) {
        resultDiv.innerHTML = `<strong style="color: #d32f2f;">Error: Edmonds-Karp requiere capacidades positivas. Ajusta las aristas y vuelve a generar el grafo.</strong>`;
        resultDiv.style.display = 'block';
        return;
    }
    ...
    try {
        maxFlowResult = edmondsKarp(currentN, currentEdges, source, sink);
        ...
        resultDiv.style.display = 'block';
        console.error('Error en Edmonds-Karp:', error);
    }
}
...
function edmondsKarp(n, edges, source, sink) {
    ...
    // Construir la matriz de capacidades para el grafo dirigido
    ...
    // Edmonds-Karp: BFS sobre el grafo residual para encontrar el camino más corto en número de aristas
    while (bfs()) {
        ...
        for (let v = sink; v !== source; v = parent[v]) {
            ...
            residual[v][u] += pathFlow; // actualizar la capacidad inversa
        }
