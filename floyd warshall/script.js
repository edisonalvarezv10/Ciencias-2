...
// Variables globales para almacenar resultados de Floyd-Warshall
let floydDistances = null;
let floydNext = null;

// Función para reconstruir el camino más corto
function reconstructPath(start, end) {
    if (floydNext === null) return null;
    if (floydNext[start][end] === null) return null;
...
function buscarCaminoMasCorto() {
    ...
    if (floydDistances === null) {
        alert('Genera las representaciones primero');
        return;
    }
    
    const distance = floydDistances[start][end];
    
    if (distance === 'No conexión') {
        ...
        const pathStr = path.map(numToLetter).join(' → ');
...
function floydWarshall(adjMatrix) {
    ...
    // Initialize: set diagonal to 0, no edge to Infinity
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < n; j++) {
            if (i === j) {
                dist[i][j] = 0;
    ...
    // Convertir Infinity a un valor grande representativo
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < n; j++) {
            if (dist[i][j] === Infinity) {
                dist[i][j] = 'No conexión';
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    generateBtn.addEventListener('click', function() {
        ...
        const floydResult = floydWarshall(matrizAdyacencia);
        const shortestPaths = floydResult.dist;
        
        // Almacenar resultados para usar en buscarCaminoMasCorto
        floydDistances = floydResult.dist;
        floydNext = floydResult.next;
        
        // Llenar selectores de nodos
        ...
        document.getElementById('shortest-paths').innerHTML = renderTable(
            shortestPaths.map((row, i) => [numToLetter(i), ...row]), 
            headersAdj
        );
